...
function addProducto(nombre, precio_por_gramo, stock_inicial, stock_minimo) {
    ...
    if (typeof _supabase !== 'undefined' && _supabase) {
        try { _supabase.from('productos').insert(p).then(function() {}).catch(function() {}); } catch (e) {}
    }
    return p;
}
...
function updateProducto(id, data) {
    ...
    guardarEstado();
    if (typeof _supabase !== 'undefined' && _supabase) {
        try { _supabase.from('productos').upsert(p, { onConflict: 'id' }).then(function() {}).catch(function() {}); } catch (e) {}
    }
}
...
function ajustarStock(productoId, cantidad_gramos, tipo) {
    ...
    if (typeof _supabase !== 'undefined' && _supabase) {
        try { _supabase.from('productos').update({ stock_gramos: p.stock_gramos }).eq('id', p.id).then(function() {}).catch(function() {}); } catch (e) {}
    }
    return true;
}
...
function registrarVentaPorGramos(productoId, cantidad_gramos) {
    ...
    if (typeof _supabase !== 'undefined' && _supabase) {
        try {
            ...
            _supabase.from('stock_movements').insert({ producto: p.id, tipo: 'venta', cantidad_gramos: -cantidad_gramos, usuario: sesionActual ? sesionActual.usuario : '?', timestamp: new Date().toISOString().slice(0, 19) }).then(function() {}).catch(function() {});
        } catch (e) {}
    }
